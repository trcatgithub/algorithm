...
public class SolutionX {
    public int[] findRedundantDirectedConnection(int[][] edges) {
        ...
        for(int i=edges.length-1; i>=0; i--) {
            ...
            if(tempParent[tempParent.length-unique] == 0 && tempParent[tempParent.length-unique+1] > 0 // 只有一个节点没有父节点(即只存在一个父节点)
                ...
                // 处理环
                if(left[0] <= 0) {
                    System.out.println("unique:  "+unique+"root: "+root+"     left: "+left[0]+"    "+Arrays.toString(tempParent));
                    return edges[i];
                }
    ...
    // 根据根节点root向下bfs，计算经过的节点数count[0]
...
//        System.out.println(Arrays.toString(new SolutionX().findRedundantDirectedConnection(edges)));
